...
with open('Girona_Boadella.txt') as file:
    raw_data = file.read()
comma_to_dot = re.compile(r',')
comma_process = comma_to_dot.sub('.', raw_data)
print(comma_process)
avoid_letters = re.compile(r'(\d+\s+\d+(\.\d+)?)')
processed_data = avoid_letters.findall(comma_process)
...
np.savetxt('prec_prd_rtrn.txt', list(zip(pr, T)), fmt="%1f", header = #!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Tue Dec  3 19:16:00 2019
