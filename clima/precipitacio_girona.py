...
# %% read data
raw_data = pd.read_csv('prec.txt', sep='\t', decimal=",",
                       index_col=['Year'], usecols=['Year', 'Pmax'])
...
FD = int((stts['Pmax']['max'] - stts['Pmax']['min']) /
         (2*(stts['Pmax']['75%']-stts['Pmax']['25%'])*stts['Pmax']['count']**(-1/3)))
